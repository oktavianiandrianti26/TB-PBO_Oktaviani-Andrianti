...
public class Pelanggan extends Mobil{ //inhiritance
    ...
    public String tglPembelian() { //method string and date
        Date date = new Date(); //variable date bertipe tanggal
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
        return dateFormat.format(date); //tanggal saat ini
    }

    public void insertDBPelanggan() throws SQLException{ //CRUD untuk memasukkan data
        ...
        Statement statement = con.createStatement(); //kemudian statementnya akan di execute dan apabila berhasil maka data berhasil di input
        statement.execute(sql);
        System.out.println("\nDATA BERHASIL DI INPUT !!!");
    }
    
    public void Show1() throws SQLException{ //CRUD untuk melihat data didatabase
        String sql = "SELECT id_mobil, merk, tipe, harga, status FROM data_mobil";
        con = DriverManager.getConnection(url,"root","");
        Statement statement = con.createStatement();
        ResultSet result = statement.executeQuery(sql);

        while(result.next()){ //perulangan (while untuk menampung berapa banyak data di database tsb)
            System.out.println("\n");
    ...
    public void Show2() throws SQLException{ //CRUD untuk menampilkan transaksi yg terjadi
    ...
    public void ubah() throws SQLException{ //CRUD
        try {
            ...
            String sql = "SELECT id_mobil, merk, tipe, harga, status FROM data_mobil  WHERE id_mobil = '"+id+"'";  
            con = DriverManager.getConnection(url,"root","");
            Statement statement = con.createStatement();
            ResultSet result = statement.executeQuery(sql);
